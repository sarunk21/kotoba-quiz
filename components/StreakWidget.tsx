...
export default function StreakWidget({ onStartPractice }: StreakWidgetProps) {
 ...
 if (!isClient || !stats) {
 return (
 <div className="rounded-3xl p-5 mb-4 border border-[var(--color-border)] bg-[var(--color-surface)] animate-pulse">
 ...
 return (
 <div
 className="rounded-3xl p-5 mb-5 border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm relative overflow-hidden transition-all anim-up"
 >
 ...
 {isTodayDone ? (
 <span className="text-[9px] font-extrabold px-2 py-0.5 rounded-full bg-[var(--color-green-light)] text-[var(--color-green)] border border-emerald-200 dark:border-emerald-800/40">
 Aktif Hari Ini ✨
 </span>
 ) : currentStreak > 0 ? (
 ...
 : d.isToday
 ? 'bg-[var(--color-surface)] text-[var(--color-accent)] border-2 border-dashed border-[var(--color-accent)]'
 : 'bg-[var(--color-surface)] text-[var(--color-text-3)] border border-[var(--color-border)] opacity-60'
 }`}
 >
 {d.active ? '✓' : d.isToday ? '•' : '○'}
 </div>
