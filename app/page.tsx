...
import { type VocabItem, loadLocalVocab } from '@/lib/vocab'
...
const StudyHeatmap = React.memo(function StudyHeatmap({ history }: { history: Record<string, number> }) {
 ...
 return { dates: arr, todayStr: getLocalDateString(today) }
 }, [history])

 const getLocalDateStringLocal = (d: Date) => getLocalDateString(d)
 ...
 {dates.map(date => {
 const dateStr = getLocalDateStringLocal(date)
 ...
 <div 
 key={dateStr} 
 className={`w-3.5 h-3.5 rounded-[4px] ${bg} transition-all duration-200 relative cursor-pointer`}
 title={`${formattedDate}: ${levelTitle}`}
 />
 )
 })}
 </div>
...
export default function Home() {
 ...
 // Load study history via storage wrapper
 const sh = getHistoryRaw()
 if (sh) {
 try {
 setStudyHistory(JSON.parse(sh))
 } catch (e) {
 console.error(e)
 }
 }
 ...
 // Account Isolation — pakai storage helper
 useEffect(() => {
 ...
 const sh2 = getHistoryRaw()
 if (sh2) {
 try {
 setStudyHistory(JSON.parse(sh2))
 } catch (e) {
 console.error(e)
 }
 }
 ...
 useEffect(() => {
 if (session?.user?.email && isAutoSync()) doSync()
 ...
 <div ref={scrollRef} className="max-w-sm md:max-w-2xl mx-auto px-4 pt-12 pb-36 overflow-y-auto"
 ...
 <div className="absolute right-0 mt-2 w-48 bg-[var(--color-surface)] rounded-2xl shadow-xl z-[70] border border-[var(--color-border)] overflow-hidden anim-pop">
 ...
 <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-3 rounded-2xl py-4 text-base font-extrabold active:scale-95 transition-transform" style={{ background: 'var(--color-accent)', color: '#fff', boxShadow: '0 8px 20px var(--color-accent-glow)' }}>
 ...
 <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[32px] p-8 w-full max-w-xs relative anim-pop shadow-2xl text-center">
 <div className="text-5xl mb-4">✨</div>
 ...
 setUpdateInfo(null)
 }} className="w-full rounded-2xl py-4 text-base font-extrabold active:scale-95 transition-transform text-white" style={{ background: 'var(--color-accent)', boxShadow: '0 8px 20px var(--color-accent-glow)' }}>
 Unduh Update 🚀
 </button>
