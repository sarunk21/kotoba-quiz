...
function StudyHeatmap({ history }: { history: Record<string, number> }) {
  ...
  const getLocalDateStringLocal = (d: Date) => {
  ...
  return (
    <div className="rounded-3xl p-5 mb-4 border border-[var(--color-border)] bg-white dark:bg-[#1a1d24] anim-up d1" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
      ...
      <div className="flex items-start gap-2 justify-center select-none">
        ...
        <div className="grid grid-flow-col grid-rows-7 gap-1.5 overflow-x-auto no-scrollbar h-[116px] pt-1">
          {dates.map(date => {
            ...
            let bg = 'bg-gray-100 dark:bg-gray-800/30'
            let levelTitle = '0 kuis'
            
            if (count > 0 && count <= 5) {
              bg = 'bg-indigo-100 dark:bg-indigo-950/40 border border-indigo-200/10'
              levelTitle = `${count} kuis`
            } else if (count > 5 && count <= 15) {
              bg = 'bg-indigo-300 dark:bg-indigo-800/80'
              levelTitle = `${count} kuis`
            } else if (count > 15) {
      ...
      <div className="flex items-center justify-end gap-1.5 mt-3 pr-2 text-[9px] font-black text-[var(--color-text-3)]">
        <span>Kurang</span>
        <div className="w-2.5 h-2.5 rounded-[3px] bg-gray-100 dark:bg-gray-800/30" />
        <div className="w-2.5 h-2.5 rounded-[3px] bg-indigo-100 dark:bg-indigo-950/40" />
        <div className="w-2.5 h-2.5 rounded-[3px] bg-indigo-300 dark:bg-indigo-800/80" />
        <div className="w-2.5 h-2.5 rounded-[3px] bg-indigo-600 dark:bg-indigo-600" />
        <span>Lebih</span>
      </div>
    </div>
  )
